...
func InitTable(db *xorm.Engine) error {
	if err := db.Sync(new(Project), new(Namespace), new(DbAccount), new(ImageAccount), new(Tenant)); err != nil {
		return err
	}
	return nil
}
...
func DropTables(db *xorm.Engine) error {
	return db.DropTables(new(Project), new(Namespace), new(DbAccount), new(ImageAccount), new(Tenant))
}
