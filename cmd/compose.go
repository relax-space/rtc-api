...
func (d *Compose) Write(project *Project, flag *Flag) error {
	...
	if project.Owner.IsKafka {
		...
		d.setKafkaEland(viper, *flag.DockerKafkaPort, ip, flag.RegistryCommon, *flag.Prefix)
	}
	if project.Owner.IsMysql {
		d.setMysql(viper, *flag.DockerMysqlPort, flag.RegistryCommon, *flag.Prefix)
	}
	if project.Owner.IsRedis {
		d.setRedis(viper, *flag.DockerRedisPort, flag.RegistryCommon, *flag.Prefix)
	}
	if project.Owner.IsSqlServer {
		d.setSqlServer(viper, *flag.DockerSqlServerPort, flag.RegistryCommon, *flag.Prefix)
	}
	if project.Owner.IsStream {
		d.setEventBroker(viper, project, *flag.ImageEnv, *flag.DockerHostIp, *flag.Prefix)
	}
	d.setNginx(viper, project, *flag.DockerNginxPort, flag.RegistryCommon, *flag.Prefix)
...
func (d *Compose) Exec(project *Project, flag *Flag) error {
	...
	if BoolPointCheck(flag.DockerNoPull) == false {
...
func (d *Compose) GetProjectImage(name, imageEnv string) string {
	image := name
	if len(imageEnv) != 0 {
		image += "-" + imageEnv
	}
...
func (d *Compose) GetCommonImage(name string, registryCommon *string) string {
	var registry string
...
func (d *Compose) setEventConsumer(viper *viper.Viper, project *Project, streamNames []string, imageEnv, prefix string) {
	for _, sName := range streamNames {
		...
		imageName := project.Setting.Image + "-" + sName
		viper.Set(servicePre+".image", d.GetProjectImage(imageName, imageEnv))
...
func (d *Compose) checkAll(project *Project, flag *Flag, dockercompose string) (err error) {
	...
	if project.Owner.IsMysql {
		if err = d.checkMysql(dockercompose, *flag.DockerMysqlPort, ip); err != nil {
			return
		}
	}
	if project.Owner.IsSqlServer {
		if err = d.checkSqlServer(dockercompose, *flag.DockerSqlServerPort, ip); err != nil {
			return
		}
	}
	if project.Owner.IsKafka {
		if err = d.checkKafka(dockercompose, *flag.DockerKafkaPort, ip); err != nil {
			return
		}
	}
	return
...
func (d *Compose) checkMysql(dockercompose, port, ip string) error {
	...
	if _, err := CmdRealtime("docker-compose", "-f", dockercompose, "up", "-d", "--no-recreate", d.getServiceServer(MYSQL.String())); err != nil {
		return err
	}
...
func (d *Compose) checkSqlServer(dockercompose, port, ip string) (err error) {
	...
	if _, err = CmdRealtime("docker-compose", "-f", dockercompose, "up", "-d", "--no-recreate", d.getServiceServer(dbType)); err != nil {
		return
	}
	Info(fmt.Sprintf("begin ping %v,%v:%v", dbType, ip, port))
...
func (d *Compose) checkKafka(dockercompose, port, ip string) (err error) {
	if _, err = CmdRealtime("docker-compose", "-f", dockercompose, "up", "-d", "--no-recreate", d.getServiceServer("zookeeper")); err != nil {
		return
	}

	if _, err = CmdRealtime("docker-compose", "-f", dockercompose, "up", "-d", "--no-recreate", d.getServiceServer("kafka")); err != nil {
		return
	}
...
func (d *Compose) setMysqlSimple(viper *viper.Viper, port string, registryCommon *string, prefix string) {

	serviceName := "mysql"
	servicePre := d.getServicePreSimple(serviceName, prefix)
	viper.Set(servicePre+".image", d.GetCommonImage("mysql:5.7.22", registryCommon))
	viper.Set(servicePre+".container_name", d.getContainerName(serviceName, prefix))
	viper.Set(servicePre+".ports", []string{port + ":" + d.InPort.Mysql})
	viper.Set(servicePre+".environment", []string{"MYSQL_ROOT_PASSWORD=1234"})
...
func (d *Compose) getServicePreSimple(serviceName, prefix string) string {
	return "services." + d.getServiceServerSimple(serviceName, prefix)
}

func (Compose) getServiceServerSimple(serviceName, prefix string) string {
	return prefix + strings.ToLower(serviceName) + SUFSERVER
}
